...
creating_node_data <- function(join_ccm_exposure_investigations_and_exposures_data) {
  create_node_data <- renaming_ccm_exposure_investigations_data(join_ccm_exposure_investigations_and_exposures_data) %>%
    select(from, to) %>%
    pivot_longer(cols = everything(), names_to = "mode", values_to = "name") %>%
    distinct(name)

  return(create_node_data)
}
